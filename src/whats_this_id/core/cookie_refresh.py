...
from whats_this_id.core.config import BROWSER_CONFIG, BROWSER_CACHE_DIR
    ...
    async def _refresh_cookies_automated(self, target_urls: List[str]) -> bool:
        ...
        try:
                ...
                for url in target_urls:
                    try:
                        ...
                        if result.success:
                            ...
                            logger.warning(f"Failed to refresh cookies for {url}: {result.error_message}")
            ...
            if success_count > 0:
                ...
                logger.info(f"Cookie refresh completed successfully for {success_count}/{len(target_urls)} sites")
                return True
            else:
...
Or use the shell script (recommended):
   ./refresh_cookies.sh
    ...
    async def refresh_cookies_if_needed(self, error_message: str, target_urls: Optional[List[str]] = None) -> bool:
        """
