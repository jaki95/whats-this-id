...
class Parser(BaseOperation):
    ...
    def __init__(self, model: str = "gpt-4o-mini", timeout: int = 15, max_retries: int = 3):
    ...
    async def _execute_async(self, content: str) -> Tuple[List[Any], float]:
        ...
        print(f"🔍 Parser: Starting HTML parsing with {len(content)} characters of content...")
        ...
        try:
            ...
            print(f"✅ Parser: Extracted {len(tracks)} tracks with confidence {confidence:.2f}")
    ...
    def _extract_tracks_from_html(self, soup: BeautifulSoup) -> List[Any]:
        ...
        if not tracks:
            track_elements = soup.find_all(['div', 'li', 'tr'], class_=re.compile(r'track|song|item', re.I))
    ...
    def _extract_track_info_from_tlp_element(self, element, soup) -> Any:
        ...
        is_id_track = element.get('data-isid') == 'true' or 'id - id' in text.lower() or ' - id' in text.lower()
        ...
        if ' - ' in text:
            ...
            if len(parts) >= 2:
                ...
                track = re.sub(r'\s+[A-Z\s]{4,}\s+\d+.*$', '', track_part)  # Remove label and numbers at end (only if label is 4+ chars)
                ...
                track = re.sub(r'\s+[A-Z]+\s+\[.*?\].*$', '', track)  # Remove text like "OFUNSOUNDMIND  [edit?] ..."
                track = re.sub(r'\s+\d+\s+[a-zA-Z0-9()\s]+\s+Save.*$', '', track)  # Remove text like "11    ewwcolton (19k)                 Save 10"
                track = track.strip()
                ...
                artist = re.sub(r'^\d+\s+\d+:\d+(?::\d+)?\s+', '', artist_part)  # Remove "01 02:30 " or "25 2:01:30 "
    ...
    def _find_suggestion_for_id_track(self, element, soup) -> tuple:
        ...
        if suggestion_container:
            ...
            suggestion = suggestion_container.find('div', id=re.compile(r'sug\d+_value'))
            if suggestion:
    ...
    def _extract_track_info_from_text(self, text: str) -> Any:
        ...
        if ' - ' in text or ' by ' in text:
            ...
            if any(skip_word in text.lower() for skip_word in ['copyright', 'rights', 'reserved', 'tracklist', 'playlist']):
                return None
