...
class TracklistManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.parser = Parser(
            timeout=self.config.get("parser_timeout", 15),  # Reduced from 60 to 15 seconds
    ...
    def run(self, query: str) -> SearchRun:
        ...
        try:
            ...
            if not query or not query.strip():
                log_step(run, "Validation", "Manager", "error", error="Query cannot be empty")
                return run
            ...
            if run.final_tracklist is not None:
                if hasattr(run.final_tracklist, 'tracks') and run.final_tracklist.tracks is not None:
    ...
    def _search_tracklists(self, query: str, run: SearchRun) -> List[Any]:
        ...
        try:
            ...
            log_step(
                run, "Search",
                "1001tracklists",
                "error", error=user_friendly_error, duration_ms=duration
            )
            return []
    ...
    def _fetch_and_parse(self, results: List[Any], run: SearchRun) -> List[tuple]:
        ...
        for result in results:
            ...
            try:
                ...
                logger.info(f"Parsed {len(tracks)} tracks with confidence {confidence:.2f}")
                ...
                log_step(
                    run, "Parse",
                    "Fetcher + Parser",
                    "error", error=user_friendly_error, link=result.link, duration_ms=duration
                )
    ...
    def _merge_results(self, parsed_tracklists: List[tuple], run: SearchRun) -> None:
        ...
        if not parsed_tracklists:
            log_step(run, "Aggregation", "Manager", "no_results", details="No tracklists to merge")
        ...
        for track in all_tracks:
            title_lower = track.name.lower() if hasattr(track, 'title') else str(track).lower()
        ...
        # Sort alphabetically
        try:
            unique_tracks.sort(key=lambda t: getattr(t, 'track_number', 999) if getattr(t, 'track_number', None) is not None else 999)
        except:
            pass  # If sorting fails, keep original order
        
        # Create a DomainTracklist object with the unique tracks
        from whats_this_id.core.common import DomainTracklist
        avg_confidence = total_confidence / len(parsed_tracklists) if parsed_tracklists else 0.0
        run.final_tracklist = DomainTracklist(
        ...
        log_step(
            run, "Aggregation", "Manager", "success",
            found_tracks=len(unique_tracks),
            confidence=avg_confidence,
            details=f"Merged {len(parsed_tracklists)} tracklists into {len(unique_tracks)} unique tracks"
        )
        
        logger.info(f"Aggregation complete: {len(unique_tracks)} unique tracks from {len(parsed_tracklists)} tracklists")
