...
"""
from whats_this_id.core import TracklistManager, SearchRun
...
def main():
    ...
    if result.success and result.final_tracklist is not None:
        tracks = result.final_tracklist.tracks if hasattr(result.final_tracklist, 'tracks') and result.final_tracklist.tracks is not None else []
        track_count = len(tracks)
        print(f"Found {track_count} tracks")
        print(f"Search completed in {result.total_duration_ms:.2f}ms")
        
        for track in tracks:
            track_num = f"{track.track_number:02d}" if track.track_number is not None else "??"
