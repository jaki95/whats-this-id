...
_api_service: Optional[DJSetProcessorService] = None
...
def get_api_service() -> DJSetProcessorService:
    ...
    if _api_service is None:
        _api_service = DJSetProcessorService()
    return _api_service
...
def display_api_error(message: str, show_service_hint: bool = True):
    ...
    if show_service_hint:
        st.error(f"Make sure the DJ set processor service is running on {AppConfig.DEFAULT_API_BASE_URL}")
